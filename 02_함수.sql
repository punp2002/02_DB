...
-- 문자열을 5번째 문자부터 검색하여 두번째 B위치 조회
SELECT INSTR('AABAACAABBAA', 'B', 5, 2) -- 10
FROM DUAL;

-- EMPLOYEE 테이블에서 사원명, 이메일, 이메일 중 @ 위치조회
...
-- 주어진 컬럼이나 문자열에 앞, 뒤, 양쪽에 있는 지정된 문자를 제거
--> 양쪽 공백 제거에 많이 사용됨
...
-- EMPLOYEE 테이블에서 사번이 홀인 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE
WHERE MOD(EMP_ID, 2) <> 0;
...
-- TRUNC(숫자 | 컬럼명 [, 위치]) : 특정 위치아래를 절삭
SELECT TRUNC(123.456) FROM DUAL; -- 123, 소수점 아래를 절삭
SELECT TRUNC(123.456, 1) FROM DUAL; -- 123.4, 소수점 첫재짜리 아래 절삭
...
-- SYSDATE : 시스템에 현재시간(년,월,일,시,분,초)을 반환
SELECT SYSDATE FROM DUAL; -- 2025-03-10 12:11:22.000

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-03-10 12:12:42.722 +0900
...
-- 숫자 -> 문자 변환 시 포맷 패턴
-- 9 : 숫자 한칸을 의미, 어려개 작성 시 오른쪽 정렬
-- 0 : 숫자 한칸을 의미, 어려개 작성 시 오늘쪽 정렬 + 빈칸 0 추가
...
SELECT TO_DATE(20240310) FROM DUAL; -- 2025-03-10 00:00:00.000
...
-- R 패턴 : 1세기 기준으로 절반(50년) 이상인 경우는 이전세기(1990년대)
...
DECODE(JOB_CODE, 'J7', '20%','J6', '15%','J5', '10%',
								'5%') 인상률, DECODE(JOB_CODE, 'J7', SALARY * 1.2, 
								'J6', SALARY *1.15,
								'J5', SALARY *1.1, SALARY * 1.05) "인상된 급여"
								FROM EMPLOYEE;

-- CASE WHEN 조건식 THEN 결과갑
-- 			WHEN 조건식 THEN 결과값
-- 			ELSE 결과값
-- END
...
-- 급여 최소값, 가장 빠른 입사일, 알파뱃 순서가 가장빠른 이메일 조회
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE;

-- 급여 최대값, 가장 최근 입사일 , 알파뱃 순서가 가장 느린 이메일 조회
