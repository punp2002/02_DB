...
-- 그룹함수는 단 한개의 결과값만 산출하기 때문에 그룹이 어려개일 경우 오류가 발생
...
SUM(SALARY) "급여 합"
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여') -- 별칭 사용 X (SELECT 절 해석 전)
...
GROUP BY DEPT_CODE; -- WHERE 절에서 구한 부서코드가 D5,D6인 데이터들을 그룹으로 묶음
...
FROM EMPLOYEE
--WHERE SALARY >= 3000000 --> 한사람의 급여가 3백만원 이상이라는 조건(요구사항 X)
GROUP BY DEPT_CODE
...
/* 집합 연산자 (SET OPERATOR)
 * 
 * -- 어려 SELECT의 결과(RESULT SET)를 하나의 결과로 만드는 연산자
 * 
