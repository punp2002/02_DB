...
-- 메인쿼리의 테이블 값이 변경 되면 서브쿼리의 결과값도 바뀌게 되는 구조

-- 상관쿼리는 먼저 메인 쿼리 한행을 조회하고
...
--> 상관 서브쿼리를 사용하여 각 직원의 MANAGET_ID가
...
-- 부서별 입사일이 가장빠른 사원의
-- 사번, 이름, 부서코드, 부서명(NULL 이라면 '소속없음').
-- 직급명, 입사일을 조회하고
-- 입사일이 빠른순으로 정렬해라
-- 단, 퇴사한 직원은 제외해라

-- 메인쿼리(사번, 이름, 부서코드, 부서명(NULL 이라면 '소속없음').
...
FROM (SELECT EMP_NAME, SALARY
			FROM EMPLOYEE
			ORDER BY SALARY DESC) -- 해석 순위 1순위 FROM절에서 전체 직원의 SALARY 내림차순 정렬
WHERE ROWNUM <= 5; -- 해석순위 2위 WHERE 절에서 가상 컬럼의 1~5행 까지만 조회
