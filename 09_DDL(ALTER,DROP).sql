...
ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_COY_TITLE_NN NOT NULL(DEPT_TITLE);
-- ORA-00904: : 부적합한 식별자
--> NOT NULL 제약조건은 새로운 조건을 추가하는 것이 아닌
-- 컬럼 자체에 NULL 허용/비허용 제어하는 성질 변경의 형태로 인식됨.
...
--> CACADE CONSTRAINTS : 삭제하려는 테이블과 연결된 FK 제약조건을 모두 삭제
...
SELECT * FROM TB2;
-- 롤백 안된다.. 위에서 DDL 구문중 ALTER를 사용해서
