...
-- 더미테이블이란 실제 데이터를 저장하는게 아닌, 임시 계산이나 테스트 목적 사용.
 ...
 * 
 * 컬럼명 AS "별칭" : 별칭 띄어쓰기 , 특수문자 , 문자만 모두 가능
 * 
 * AS 생략 가능
 * 
 * */
...
-- DISTINCT : 조회 시 컬럼에 포함된 중복 값을 한번만 표기할때 사용
...
-- CTRL + SHIFT + 위아래방향기 : 줄 위아래 이동
-- CTRL + ALT + 위아래방향기 : 줄 위아래 복사
...
-- EMPLOYEE 테이블에서 이메일에 _앞에 글자가 3글자인 사원만 
-- 이름 이메일 조회
...
-- EMPLOYEE 테이블에서
-- 이메일 '_' 앞이 4글자 이면서
-- 부서코드가 'DP' 또는 'D6' 이고 -- >> AND가 OR 보다 우선 순위가 높다, () 사용 가능
...
-- EMPLOYEE 테이블에 보너스가 있는 사원의 이름, 보너스 조회
...
-- EMPLOYEE 테이블에 보너스가 없는 사원의 이름, 보너스 조회
...
FROM EMPLOYEE
ORDER BY SALARY; -- ASC(오름차순)가 기본 값 [ASC | DESC]
-- 컬럼명 사용.
